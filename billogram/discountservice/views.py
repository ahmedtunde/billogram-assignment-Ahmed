...
from billogram.authentication import AllowAnyUser
from billogram.discountservice import models, serializers
...
schema_view = get_schema_view(
    openapi.Info(
        ...
        contact=openapi.Contact(email='bab44un@yahoo.com'),
    ),
    public=True,
...
class DiscountCodeViewSet(CreateModelMixInWithObjectPermissionCheck, ViewMixIn, ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        if self.request.user and self.request.user.is_staff:
            ...
            discount=models.DiscountCode.objects.filter(rule__brand=self.request.user.brand)
            return discount
        else:
