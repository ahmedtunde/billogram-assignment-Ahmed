...
from billogram.discountservice.models import User as User
...
class DiscountCodeViewSetTest(TestCase):
    def setUp(self) -> None:
        ...
        self.brand = models.Brand.objects.create(name='Billogram')
        self.brand_user = User.objects.create_user(username='Ahmed', is_brand=True, brand=self.brand)
        ...
        self.discount_code = models.DiscountCode.objects.create(
            id='DISCOUNT001', rule=self.discount_rule, valid_till=date(2022, 11, 30))
    ...
    def test_retrieve(self):
        response = self.client.get(reverse('code-detail', kwargs=dict(pk='testingpk')))
...
class DiscountCodeUsageViewSetTest(TestCase):
    def setUp(self) -> None:
        ...
        self.brand_user.save()
        self.brand_client = get_logged_in_client(self.brand_user)
        self.other_brand = models.Brand.objects.create(name='Games')

        self.discount_rule = models.DiscountRule.objects.create(brand=self.brand, discount=50)
        self.discount_rule_2 = models.DiscountRule.objects.create(brand=self.other_brand, discount=25)
        self.discount_code = models.DiscountCode.objects.create(
            id='billo2022', rule=self.discount_rule, valid_till=date(2022, 11, 30))
        self.discount_code_2 = models.DiscountCode.objects.create(
            id='billo2023', rule=self.discount_rule_2, valid_till=date(2022, 11, 30)
        )
    ...
    def test_create(self):
        ...
        new_discount_code = models.DiscountCode.objects.create(
            id='new2022', rule=self.discount_rule, valid_till=date(2022, 11, 30))
        data = {
